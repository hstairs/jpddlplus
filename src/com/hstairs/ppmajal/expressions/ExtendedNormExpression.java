...
import jpddlplus.conditions.ComplexCondition;
    ...
    for (final ExtendedAddendum a : this.summations) {
      for (final ExtendedAddendum b : right.summations) {
        ...
        if (a.linear && a.f == null && b.linear && b.f == null) {
          ...
          ele_to_add.bin = new BinaryOp(new PDDLNumber(a.n * b.n), "*",
              new BinaryOp(a.f, "*", b.f, true), true);
          ele_to_add.linear = false;
        } else if (!a.linear && b.linear && b.f == null) {
          ele_to_add.bin = new BinaryOp(a.bin.clone(), "*", new PDDLNumber(b.n), true);
          ele_to_add.linear = false;
        } else if (!a.linear && b.linear && b.f != null) {
          ele_to_add.bin = new BinaryOp(a.bin.clone(), "*",
              new BinaryOp(new PDDLNumber(b.n), "*", b.f, true), true);
          ele_to_add.linear = false;
        } else if (a.linear && a.f == null && !b.linear) {
          ele_to_add.bin = new BinaryOp(b.bin.clone(), "*", new PDDLNumber(a.n), true);
          ele_to_add.linear = false;
        } else if (a.linear && a.f != null && !b.linear) {
          ele_to_add.bin = new BinaryOp(new BinaryOp(new PDDLNumber(a.n), "*", a.f, true), "*",
              b.bin.clone(), true);
          ele_to_add.linear = false;
    ...
    for (ExtendedAddendum a : this.summations) {
      for (ExtendedAddendum b : right.summations) {
        ...
        if ((a.linear && a.f == null && b.linear && b.f == null)) {
          ...
          ele_to_add.bin = new BinaryOp(new PDDLNumber(a.n), "/",
              new BinaryOp(new PDDLNumber(b.n), "*", b.f, true), true);
          ele_to_add.linear = false;
...
//                Float temp =  a.bin.eval(s).getNumber();
  ...
  public ExtendedNormExpression weakEval(PDDLProblem problem, Set invFluents) {
    ...
    Double c = 0d;
    for (ExtendedAddendum a : this.summations) {
      if ((a.bin != null && !a.linear) || a.n == null) {
        ...
        if (a.n == null) {
          throw new RuntimeException(
              "Something didn't work in the weak evaluator for normalised expression: " + this);
        }
        if (a.f != null) {
...
//                System.out.println("BAaaaaa... c: "+c+"a: "+a);
  ...
  public ExtendedNormExpression subst(Condition input) {
    if (!(input instanceof ComplexCondition numeric)) {
      return this;
    }
...
//                ret = IAMath.add(ret,new Interval1(a.n.floatValue()));
  ...
  public Double getNumber() {
    if (this.isNumber()) {
      ...
      System.out.println(this + "  is not a single number");
      return null;
    }
...
//                System.out.println(ad.bin);
  ...
  public float eval_not_affected(PDDLState s_0,
      TransitionGround action) {//this applies only to linear expression. In the other cases the behavior is undefined
    float current = 0;
  ...
  public float eval_affected(PDDLState s_0,
      TransitionGround aThis) {//this applies only to linear expression. In the other cases the behavior is undefined
    float current = 0;
    for (ExtendedAddendum ad : this.summations) {
      if (ad.f != null) {
  ...
  public float eval_apart_from_f(NumFluent f,
      PDDLState s) {//this applies only to linear expression. In the other cases the behavior is undefined
    float ret = 0;
    for (Object o : this.summations) {
      ExtendedAddendum a = (ExtendedAddendum) o;
      if (a.f != null) {
