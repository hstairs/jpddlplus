...
import jpddlplus.domain.ActionParameter;
...
public class NumFluent extends Expression {
  ...
  public String toString() {
    ...
    for (Object o1 : this.getTerms()) {
      ...
      if (o1 instanceof PDDLObject obj) {
  ...
  public Expression unGround(Map substitution) {
    ...
    for (Object o : terms) {
      if (o instanceof PDDLObject obj) {
        Variable t = (Variable) substitution.get(obj.getName());
        if (t == null) {
          System.out.println("Substitution Failed for " + o);
          System.exit(-1);
        } else {
          variables.add(t);
        }
...
//    }
  ...
  public HomeMadeRealInterval eval(RelState s) {
    ...
    HomeMadeRealInterval ret = new HomeMadeRealInterval(s.functionInfValue(this),
        s.functionSupValue(this));
  ...
  public boolean isUngroundVersionOf(Expression expr) {
    if (expr instanceof NumFluent nf) {
