...
public class H1Fix extends H1 {
  ...
  private final boolean superFix = false;
  final private boolean saferVersion;
  final boolean[] visitedActions;
  private final boolean hmrpEstimate;

  public H1Fix(PDDLProblem problem, boolean saferVersion, boolean maxHelpfulTransitions,
      String redConstraints,
      boolean helpfulActionsComputation, boolean reachability, boolean helpfulTransitions,
      boolean conjunctionsMax, boolean hmprpEstimate) {
    super(problem, true, true, maxHelpfulTransitions, redConstraints, helpfulActionsComputation,
        reachability, helpfulTransitions, conjunctionsMax);
  ...
  protected float computeEstimateForStore(State gs) {
    float computeEstimate = super.computeEstimateForStore(
        gs); //To change body of generated methods, choose Tools | Templates.
...
//        Arrays.fill(minNumRepetition, Integer.MAX_VALUE);
    ...
    while (!stack.isEmpty()) {
      ...
      if (!visitedActions[a]) {
        ...
        for (final int conditionId : (Collection<Integer>) getActivatingConditions(
            cp.preconditionFunction()[a]).getFirst()) {
          if (!getConditionInit()[conditionId]) {
            final int b = establishedAchiever[conditionId];
            maxNumRepetition[b] = (int) Math.max(maxNumRepetition[b],
                ceil(numRepetition[conditionId]));
//                            minNumRepetition[b] = (int) Math.min(minNumRepetition[b],ceil(numRepetition[conditionId]));
    ...
    for (idx = DG.vertexSet().size() - 1; idx >= 0; idx--) {
      ...
      for (final int conditionId : (Collection<Integer>) getActivatingConditions(
        ...
        if (terminal instanceof Comparison c) {
          float cum = 0;
//                        for (int j=0; j < idx ; j++){
...
//                            System.out.println(printTransition(b));
          ...
          if (T < 0) {
            if (saferVersion) {
              ...
              if (achiever
                  != -1) { //this is the case where there is an achiever already in the plan
                int repetition = (int) ceil(-1f * (T / numericContribution(achiever, c)));
                if (repetitionsInThePlan[achiever].size() == 1) {
                  repetitionsInThePlan[achiever] = new IntArraySet(
                      Collections.singleton(maxNumRepetition[achiever] + repetition));
                }
  ...
  private IntArraySet computeDescendants(int actionId, IntArraySet[] prec,
      IntArraySet[] descendants) {
