...
public final class Aibr extends SearchHeuristic {
  ...
  public Aibr(PDDLProblem problem, boolean reachability) {
    ...
    for (final TransitionGround tr : array) {
      generateNumericSupporters(tr, supporter2transitionMap, asymptoticPreconditionFunctionMap,
          numEffectMap);
    ...
    supporter2transitionMap.forEach(
        (integer, integer1) -> supporter2transition[integer] = integer1);
    ...
    asymptoticPreconditionFunctionMap.forEach(
        (integer, condition) -> supporter2aymptoticeffects[integer] = condition);
...
//        out.println("AIBR :: Number of Supporters = " + numberOfSupporters);
  ...
  void generatePropositionalAction(TransitionGround tr, Int2IntArrayMap supporter2transitionMap,
      Int2ObjectMap<Collection<Terminal>> propEffectMap) {
  ...
  boolean generateNumericSupporters(TransitionGround tr, Int2IntArrayMap supporter2transitionMap,
      Int2ObjectMap<Condition> asymptoticPreconditionFunctionMap,
      Int2ObjectMap<NumEffect> numEffectMap) {
    ...
    for (NumEffect effect : tr.getAllNumericEffects()) {
      ...
      if ("assign".equals(effect.getOperator()) && effect.getRight().getInvolvedNumericFluents()
          .isEmpty()) {
        ...
        if (empty) {
          ...
          if (right > 0 && effect.getOperator().equals("increase")
            ...
            generateInfSupporter(effect, supporter2transitionMap,
                supporter2transitionMap.keySet().size(), "+", asymptoticPreconditionFunctionMap,
                numEffectMap, tr);
          } else {
            generateInfSupporter(effect, supporter2transitionMap,
                supporter2transitionMap.keySet().size(), "-", asymptoticPreconditionFunctionMap,
  ...
  private void generateInfSupporter(NumEffect effect, Int2IntArrayMap supporter2transitionMap,
      int idx, String s, Int2ObjectMap<Condition> asymptoticPreconditionFunctionMap,
      Int2ObjectMap<NumEffect> numEffectMap, TransitionGround tr) {
    String inequality = "";
    ...
    generateSupporter(effect, idx, inequality, asymptote, asymptoticPreconditionFunctionMap,
        numEffectMap);
  ...
  private void generateSupporter(NumEffect effect, int idx, String inequality, Float asymptote,
    ...
    indirectPrecondition = (Comparison) Comparison.comparison(inequality, left, new PDDLNumber(0),
        false).normalize();
  ...
  @Override
  protected float computeEstimateForStore(State s0) {
    final PDDLState s = (PDDLState) s0;
    final RelState relState = s.relaxState();
    final IntArraySet supporters = new IntArraySet(
        ContiguousSet.create(closedOpen(0, numberOfSupporters), DiscreteDomain.integers()));
    ...
    if (DEBUG) {
      ...
      for (int ele = 0; ele < numberOfSupporters; ele++) {
        ...
        System.out.println("Precondition: " + Transition.getTransition(supporter2transition[ele])
            .getPreconditions());
    ...
    while (!supporters.isEmpty()) {
      ...
      while (iterator.hasNext()) {
        ...
        final TransitionGround tr = (TransitionGround) Transition.getTransition(
            supporter2transition[current]);
...
////                    }
      ...
      if (numAppliers.isEmpty() && propAppliers.isEmpty() && !relState.satisfy(
          problem.getGoals())) {
        if (DEBUG) {
    ...
    if (goalReached) {
      ...
      if (DEBUG) {
        System.err.println(
            "Computing actual estimate using the following transitions: " + reachableTransitions);
      }
  ...
  public Collection<TransitionGround> getAllTransitions() {
    if (reachableTransitions == null) {
      throw new RuntimeException(
          "Helpful Transitions can only be activatated in combination with the relaxed plan extraction");
    }
