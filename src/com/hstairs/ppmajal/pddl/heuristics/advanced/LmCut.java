...
public class LmCut extends H1 {
    ...
    record JGraph(IntArrayList[][] E, Collection<Integer>[] ERev) {
    }
    ...
    Float updateJG(JGraph jg, State gs, IntCollection changedActions) {
        ...
        for (final int actionId : changedActions) {
            if (jg.E[pcf[actionId]][actionId] != null) { // This shouldn't happen. There are cuts which are generated by reversed edged which are outdated.
                for (var v : jg.E[pcf[actionId]][actionId]) {
...
//        allAchievers = new IntArrayList[getTotNumberOfTerms()];
        ...
        return Pair.of(new JGraph(edges, reverseEdges), getActionHCost()[cp.goal()]);
    }
    ...
    protected float computeRepetitions(Comparison comparison, float contribution, State state) {
        ...
        final double eval = evalComparison[comparison.getId()] ;
