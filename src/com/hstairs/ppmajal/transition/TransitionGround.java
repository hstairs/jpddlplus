...
public class TransitionGround extends Transition {
  ...
  public TransitionGround(String name, Semantics semantics, List<PDDLObject> parameters,
  ...
  public TransitionGround(ArrayList<NumEffect> numEffect) {
    this("waiting", Transition.Semantics.PROCESS, null, null, new ConditionalEffects(),
    ...
    if (this.sdac == null) {
      ...
      if (metric != null && metric.getMetExpr() != null) {
        if (!sdacEnabled) {
          ...
          if ((exprImpact < 0 && metric.getOptimization().equals("maximize")) || (exprImpact > 0
              && metric.getOptimization().equals("minimize"))) {
          ...
          if ((exprImpact < 0 && metric.getOptimization().equals("maximize")) || (exprImpact > 0
              && metric.getOptimization().equals("minimize"))) {
            this.sdac.add(Pair.of(BoolPredicate.getPredicate(BoolPredicate.trueFalse.TRUE),
                getImpact(exprImpact, metric.getOptimization())));
          }
...
//        }
  ...
  public static TransitionGround waitingAction() {
    if (waiting == null) {
      waiting = new TransitionGround("------>waiting", Transition.Semantics.PROCESS,
          new ArrayList<>(), null, new ConditionalEffects(), new ConditionalEffects());
    }
    return waiting;
  }
