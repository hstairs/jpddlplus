...
public abstract class Transition {
        ...
        if (transition.getName().equals(actionName)) {
          if ((((TransitionGround) transition).parameters.size() == objects.size())) {
            ...
            for (int i = 0; i < ((TransitionGround) transition).parameters.size(); i++) {
              if (!((TransitionGround) transition).parameters.get(i).getName()
  ...
  protected Transition(String name, ConditionalEffects conditionalPropositionalEffects,
      ConditionalEffects conditionalNumericEffects, Condition preconditions, Semantics semantcis) {
    this.name = name;
  ...
  public Double getCoefficientAffected(NumFluent f) {
    ...
    for (Object c : this.getConditionalNumericEffects().getAllEffects()) {
      ...
      if (nEff.getOperator().equals("increase") || nEff.getOperator().equals("decrease")) {
        coefficientAffected.put(nEff.getFluentAffected(),
            1 + right.getCoefficient(nEff.getFluentAffected()));
      } else {
        coefficientAffected.put(nEff.getFluentAffected(),
            right.getCoefficient(nEff.getFluentAffected()));
      }
    }
  ...
  public Collection<? extends NumFluent> getNumFluentsNecessaryForExecution() {
    ...
    for (NumEffect e : this.getConditionalNumericEffects().getAllEffects()) {
  ...
  public Collection<Terminal> getAllAchievableLiterals() {
    if (achievableLiterals == null) {
      ...
      final Set<Map.Entry<Condition, Collection>> set = this.conditionalPropositionalEffects.getActualConditionalEffects()
          .entrySet();
