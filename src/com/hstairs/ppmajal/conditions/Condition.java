...
public abstract class Condition extends PDDLProblemComponent {
  ...
  /**
   * Substitutes the variables in this conditions with the PDDLObjects associated to each variable
   * in the specified table. The returned object is therefore grounded. This method may fail if
   * there is no substitution for some variable.
   *
   * @param substitution the map that indicates what object should replace the specified variable.
   * @param objects      These are the objects of the problem
   * @return a copy of this conditions where each variable is replaced to the object according to
...
//    public abstract void changeVar (Map substitution);
   ...
   *
   * @param vars the list where the variables involved in this condition are to be stored.
   */
  ...
  public boolean mutual_exclusion_guaranteed(Condition preconditions) {
    if (preconditions instanceof AndCond a && this instanceof AndCond b) {
      ...
      for (int i = 0; i < c1.size(); i++) {
        for (int j = i + 1; j < c2.size(); j++) {
          ...
          if (a_1 instanceof NotCond nc && a_2 instanceof BoolPredicate) {
            if (nc.getSon().equals(a_2)) {
              return true;
            }
          } else if (a_2 instanceof NotCond nc && a_1 instanceof BoolPredicate) {
