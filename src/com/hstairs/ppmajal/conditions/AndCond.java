...
public class AndCond extends ComplexCondition implements PostCondition {
  ...
  public String toSmtVariableString(int i) {
    ...
    if (this.sons != null) {
      ...
      for (Object o : this.sons) {
        if (o instanceof BoolPredicate p) {
          ret += " " + p.toSmtVariableString(i);
        } else if (o instanceof NotCond nc) {
        ...
        } else if (o instanceof NumEffect neff) {
  ...
  public Condition weakEval(PDDLProblem s, Set invF) {
    ...
    if (this.sons != null) {
      ...
      while (it.hasNext()) {
        ...
        } else if (o2 instanceof NumEffect ne) {
      ...
      for (Object o : this.sons) {
        if (o instanceof BoolPredicate p) {
          ...
          throw new UnsupportedOperationException(
              "Num effect not supported for repetition.." + this);
        }
      }
      if (this.sons.length > 1) {
        ret += ")";
    ...
    for (Object c1 : this.sons) {
      if (c1 instanceof Condition) {
        ...
        if (res instanceof AndCond) {
          Collections.addAll(newSons, ((AndCond) res).sons);
        } else {
          newSons.add(c1);
        }
      }
...
//                    ) {
      if (o instanceof PostCondition pc) {
...
//                ret_val = ret_val.concat(comp.pddlPrint(typeInformation));
  ...
  public ComplexCondition and(Condition precondition) {
    ...
    Collections.addAll(res, this.sons);
...
//        AndCond ret = new AndCond();
  ...
  public static Comparison generateRedConstraints(Comparison a1, Comparison a2) {
    ...
    Comparison newC = (Comparison) Comparison.comparison(new_comparator, expr.normalize(),
        new ExtendedNormExpression(0d), false).normalize();
    if (newC == null) {
      return null;
    }
