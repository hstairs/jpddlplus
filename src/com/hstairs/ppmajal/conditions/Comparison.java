...
public class Comparison extends Terminal {
  ...
  public static Comparison comparison(String comparator, Expression left, Expression right,
      boolean normalized) {
  ...
  public Condition ground(Map<Variable, PDDLObject> substitution, PDDLObjects po) {
    Comparison ret = comparison(comparator, left.ground(substitution, po),
        right.ground(substitution, po), false);
    return ret;
  }
  ...
  public double distanceUntilSatisfied(State s) {
    Double first = left.eval(s);
    Double second = right.eval(s);
    return switch (this.getComparator()) {
      case "<", "<=" -> Utils.doubleComparator(first, second) < 0 ? 0 : Math.abs(first - second);
      case ">", ">=" -> Utils.doubleComparator(first, second) > 0 ? 0 : Math.abs(first - second);
      case "=" -> Math.abs(first - second);
      default -> throw new RuntimeException(this.getComparator() + "  is not supported");
    };
  }
...
//        return s.satisfy(this);
  ...
  public boolean canBeTrue(RelState s) {
    ...
    if ((Double.isNaN(first.lo())) || (Double.isNaN(first.hi())) || (Double.isNaN(second.lo()))
        || (Double.isNaN(second.hi()))) {
      return false;//negation by failure.
    }
    if (this.getComparator().equals("<")) {
...
//            return !((first.lo() > second.hi()) || (second.lo() > first.hi()));
      ...
      return !((Utils.doubleComparator(first.lo(), second.hi()) > 0) || ((
          Utils.doubleComparator(second.lo(), first.hi()) > 0)));
    } else {
      System.out.println(this.getComparator() + "  is not supported");
    }
...
//    }
  ...
  private Comparison normalizeAndCopy() throws CloneNotSupportedException {
    if (normalized) {
      this.linear = ((ExtendedNormExpression) this.left).linear
          && ((ExtendedNormExpression) this.right).linear;
      return this;
    }
    String comp = comparator;

    //System.out.println("Instanceof left: "+ret.left.getClass());
    ExtendedNormExpression leftExpr = this.left.normalize();
    //System.out.println(leftExpr);
    ExtendedNormExpression rightExpr = this.right.normalize();
...
//        throw new RuntimeException("Do not clone Comparison!");
  ...
  public void computeMaxDist(RelState numericFleuntsBoundaries) {
    ...
    if ((this.getRight() instanceof ExtendedNormExpression)
        && (this.getLeft() instanceof ExtendedNormExpression lExpr)) {
      ...
      for (ExtendedAddendum a : lExpr.summations) {
        if (a.f == null) {
          ...
          num += Math.max(Math.abs(a.n * numericFleuntsBoundaries.functionInfValue(a.f)),
              Math.abs(a.n * numericFleuntsBoundaries.functionSupValue(a.f)));
        }
      }
  ...
  public boolean couldBePrevented(
      HashMap<NumFluent, HashSet<NumFluent>> computeFluentDependencePlanDependant) {
    throw new UnsupportedOperationException(
        "Not supported yet."); //To change body of generated methods, choose Tools | Templates.
  }


  public ArrayList<NumFluent> susbtFluentsWithTheirInvariants(
      HashMap<Object, Boolean> invariantFluent, int j) {
  ...
  public String toSmtVariableString(int i) {
    return "(" + this.comparator + " " + this.getLeft().toSmtVariableString(i) + " "
        + this.getRight().toSmtVariableString(i) + ")";
  }
...
//        System.out.println("rvalueClass after" + rValue.getClass());
    ...
    if (lValue instanceof PDDLNumber left && rValue instanceof PDDLNumber right) {
...
//        System.out.println(rValue);
  ...
  public Condition transformEquality() {
    ...
    if (comp.getComparator().equals("=")) {
      ...
      final Comparison dual = (Comparison) Comparison.comparison(">=", left, right, false)
          .normalize();
      final Comparison dual2 = (Comparison) Comparison.comparison("<=", left, right, false)
          .normalize();
      ret.add(dual);
      ret.add(dual2);
  ...
  public String pddlPrintWithExtraObject() {
    return "(" + getComparator() + " " + getLeft().pddlPrint(false) + " " + getRight().pddlPrint(
        false) + ")";
  }
  ...
  public boolean canBeFalse(RelState s) {
    ...
    if (this.getComparator().equals("<")) {
      ...
      return (Utils.doubleComparator(first.hi(), second.lo()) > 0
          || Utils.doubleComparator(first.lo(), second.hi()) < 0);
...
//>>>>>>> daan
  ...
  public boolean isLinear() {
    this.linear =
        ((ExtendedNormExpression) this.left).linear && ((ExtendedNormExpression) this.right).linear;
    return linear;
  }
