...
public class ConditionalEffectAsACondition extends Condition implements PostCondition {
  ...
  public ConditionalEffectAsACondition(Condition lhs, PostCondition rhs) {
    ...
    if (rhs instanceof AndCond) {
      Collections.addAll(res, ((AndCond) rhs).sons);
  ...
  public Condition clone() {
    return new ConditionalEffectAsACondition(activation_condition.clone(),
        (PostCondition) effect.clone());
  }

  public String toString() {
    if (this.activation_condition != null) {
      return "(when " + this.activation_condition.pddlPrint(true) + " " + this.effect.pddlPrint(
          true) + ")";
    }
    return null;
  }
  ...
  public Condition weakEval(PDDLProblem s, Set invF) {
    ...
    if (this.effect instanceof Condition con) {
      this.effect = (PostCondition) con.weakEval(s, invF);
      if (this.effect == null) {
        return null;
      }
    } else if (this.effect instanceof ConditionalEffectAsACondition sub) {
      this.effect = (PostCondition) sub.weakEval(s, invF);

    } else if (this.effect instanceof NumEffect ne) {
      this.effect = (PostCondition) ne.weakEval(s, invF);
    }
    return this;
  }

  public ConditionalEffectAsACondition ground(Map<Variable, PDDLObject> substitution,
      PDDLObjects po) {
...
//    }
  ...
  public String pddlPrintWithExtraObject() {
    return "(when " + this.activation_condition.pddlPrintWithExtraObject() + " "
        + this.effect.pddlPrintWithExtraObject() + ")";
  }
