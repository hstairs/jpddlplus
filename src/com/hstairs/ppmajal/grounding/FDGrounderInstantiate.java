...
public class FDGrounderInstantiate extends ExternalGrounder {
  ...
  public Collection<TransitionGround> doGrounding() {
    ...
    try {
      String fdtranslator = "python3 " + new File(
          FDGrounderInstantiate.class.getProtectionDomain().getCodeSource().getLocation()
              .toURI()).getParent() + "/../../downward/translate/instantiate.py";
      String command =
          fdtranslator + " --dump-task " + dom_prob.getFirst() + " " + dom_prob.getSecond();
      System.out.printf("DEBUG: domain file: %s problem file %s%n", dom_prob.getFirst(),
          dom_prob.getSecond());
...
//        System.out.println(groundings.keySet());
  ...
  private Pair<String, String> abstractNumeric() {
    ...
    try {
      ...
      String predicates = String.valueOf(buildPredicateString(domain.getPredicates())) +
          buildFunctionsString(domain.getFunctions());
      String toWrite = String.format(domainInput, buildTypes(domain.getTypes()),
          predicates, actionString);
...
//            File tmpFile = new File("domaincassic.pddl");
    ...
    try {
      String problemInput = "(define (problem temporary)\n (:domain temporary) "
          ...
          + ";init \n (:init %s)\n"
          + ";goals \n (:goal %s) \n )";
      String objects = problem.getObjects().pddlPrint();
      String goals = String.valueOf(getPreconditionString(problem.getGoals(), true));
...
//           }
      ...
      return (new StringBuilder(String.format(template, str)));
    }
  ...
  private StringBuilder getEffectString(ConditionalEffects conditionalPropositionalEffects,
      ConditionalEffects conditionalNumericEffects, Collection<ForAll> forallEffects) {
    StringBuilder str = new StringBuilder();
    String template0 = "(and %s )";

    StringBuilder effects = new StringBuilder();
    effects.append(
        fromConditionalEffects(conditionalPropositionalEffects.getActualConditionalEffects()));
    effects.append(fromConditionalEffects(conditionalNumericEffects.getActualConditionalEffects()));
    effects.append(
        fromUnconditionalEffects(conditionalPropositionalEffects.getUnconditionalEffect()));
  ...
  private StringBuilder fromConditionalEffects(
      Map<Condition, Collection> actualConditionalEffects) {
    StringBuilder effects = new StringBuilder();
