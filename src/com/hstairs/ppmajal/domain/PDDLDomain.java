...
public final class PDDLDomain {
  ...
  private Collection<TransitionSchema> ActionsSchema;
  private String name;
  ...
  public PDDLDomain() {
    this(new LinkedHashSet(), new PDDLObjects(), new LinkedHashSet(), new ArrayList(),
        new PredicateSet());
  }

  private PDDLDomain(Collection<TransitionSchema> eventsSchema, PDDLObjects constants,
      Set<TransitionSchema> processesSchema, Collection<NumFluent> derived_variables,
      PredicateSet p) {
  ...
  /**
   * @param file - the path of the pddl file representing the domain. As return the object will be
   *             fullfilled with the information in the pddl domain file
   * @throws IOException
...
//            System.out.println(this.constants);
      fc = new FactoryConditions(this.predicates, (LinkedHashSet<Type>) this.getTypes(),
          this.constants);
...
//                    break;
  ...
  /**
   * Returns the action with specified name. Notice that this method is rather inefficient if there
   * are many actions, and that a table that maps names to action schemas could be more effective.
   *
   * @param name the name of the action
   * @return an ActionSchema object (if any) with the name in input this assumes that there is a 1:1
   * relation between action and name, i.e. we cannot have different actions with the same name
   */
  ...
  private void addConstants(Tree c) {
    ...
    for (int i = 0; i < c.getChildCount(); i++) {
      if (c.getChild(i).getChildCount() > 0) {
        PDDLObject o = PDDLObject.object(c.getChild(i).getText(),
            Type.type(c.getChild(i).getChild(0).getText()));
...
//                        System.out.print(variable);
        ...
        case (PddlParser.EFFECT):
          ...
          forall = fc.createEffectsFromPostCondition(infoAction.getChild(0), res, propEffect,
              numEffect);
    ...
    TransitionSchema transition = new TransitionSchema(par, transitionName, propEffect, numEffect,
        precondition, semantics, forall);
    switch (semantics) {
      case ACTION:
...
//    }
  public void saveDomainWithInterpretationObjects(String file) throws IOException {
    ...
    {
      f.write(
          "(:predicates (true) " + domain.getPredicates().pddlPrintWithExtraObject(true) + " \n");
    }
  ...
  private ArrayList<TransitionSchema> pushNotAtTheTerminals(Collection<TransitionSchema> input) {
    ...
    while (iterator.hasNext()) {
      ...
      final TransitionSchema transitionSchema = new TransitionSchema(next.getParameters(),
          next.getName(),
  ...
  public PDDLDomain clone() {
    ...
    PDDLDomain res = new PDDLDomain(new LinkedHashSet(this.getEventsSchema()),
        new PDDLObjects(this.constants), new LinkedHashSet<>(this.getProcessesSchema()),
        new LinkedHashSet<>(this.derived_variables), predicates);
