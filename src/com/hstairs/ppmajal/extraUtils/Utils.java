  ...
  public static String toPDDLWithExtraObject(Collection<TransitionSchema> c) {
    ...
    for (TransitionSchema o : c) {
      throw new UnsupportedOperationException(
          "Implement writer with extra object, for whatever that means");
...
//    }
      ...
      for (NumFluent p : NumFluent.numFluentsBank.values()) {
        if (p.isGrounded()) {
          ...
          if (!Double.isNaN(fluentValue)) {
            ret.append("(= ").append(p.pddlPrint(false)).append(" ").append(fluentValue)
                .append(")");
          }
  ...
  public static void saveProblem(PDDLProblem p, PDDLState s, String pddlNewFile)
      throws IOException {
